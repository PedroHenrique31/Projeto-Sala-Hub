...
if ((!preg_match("/^[A-Za-zÀ-ú \']+$/", $nomeAgenda)) || (!preg_match("/^\S(.*\S)?$/", $atividadeAgenda)) || (!in_array($cursoAgenda, $cursosPermitidos)) ||
    (strlen($nomeAgenda) < 3) || (strlen($nomeAgenda) > 30) || (strlen($atividadeAgenda) < 5) || (strlen($atividadeAgenda) > 50) || (!preg_match("/^\d{4}\-(0[1-9]|1[012])\-(0[1-9]|[12][0-9]|3[01])$/", $dataAgenda)) ||
...
$resultT = mysqli_query($conexao, "SELECT SUM(bancadaTechAgenda) AS somaT FROM tbagenda WHERE dataAgenda = '$dataAgenda' AND horaAgenda = '$horaAgenda'");
...
$resultG = mysqli_query($conexao, "SELECT SUM(bancadaGeralAgenda) AS somaG FROM tbagenda WHERE dataAgenda = '$dataAgenda' AND horaAgenda = '$horaAgenda'");
        ...
        <div class="mt-5 d-flex justify-content-center">
            <div class="rounded-4 p-4 border border-4 shadow-sm">
                ...
                if ($bancadaTechAgenda == 0 && $bancadaGeralAgenda == 0) { ?>
                    ...
                    <!-- Se houver disponibilidade, inserir dados -->
                    <?php } else if (($availableT - $bancadaTechAgenda) >= 0 && ($availableG - $bancadaGeralAgenda) >= 0) {
                    ...
                    ?>
                        ...
                        <div class="text-center">
                            ...
                            <div class="d-flex flex-row mt-3 align-middle"><h5 class="align-middle">Monitor:&nbsp</h5><h5 style="font-weight: normal;"><?= $nomeMonitor ?></h5></div>
                            <div class="d-flex flex-row"><h5>Telefone:&nbsp</h5><h5 style="font-weight: normal;"><?php echo format_string('(##) #####-####', $str); ?></h5></div>
