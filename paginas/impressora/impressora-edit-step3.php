...
if ($_SESSION['administrador'] != 1) { $nomeImpressora = $_SESSION['nome']; }
...
$rsSecurity = mysqli_query($conexao, $sqlSecurity) or die("Erro na recuperação dos dados do registro. " . mysqli_error($conexao));
$dadosSecurity = mysqli_fetch_assoc($rsSecurity);
    if ($dadosSecurity['idUsuario'] != $_SESSION["usuario_id"] && $_SESSION['administrador'] != 1) {
        header('Location: index.php?menuop=impressora');
        exit();
    }
...
$sql = "SELECT idImpressora, dataImpressora, deHoraImpressora, ateHoraImpressora, qualImpressora 
        FROM tbimpressora  WHERE dataImpressora = '$dataImpressora' AND idImpressora != '{$idImpressora}'";
        ...
        <div class="mt-5 d-flex justify-content-center">
            <div class="rounded-4 p-4 border border-4 shadow-sm">
                ...
                if ($deHoraImpressora == 0 || $ateHoraImpressora == 0  || $deHoraImpressora >= $ateHoraImpressora) { ?>
                    ...
                    <div class="text-center">
                        <form action="index.php?menuop=impressora-edit-step2" method="post">
                ...
                if ($available1) {
                    $sql = "UPDATE tbimpressora SET 
                    nomeImpressora = '{$nomeImpressora}',
                    cursoImpressora = '{$cursoImpressora}',
                    semestreImpressora = '{$semestreImpressora}',
                    dataImpressora = '{$dataImpressora}',
                    deHoraImpressora = '{$deHoraImpressora}',
                    ateHoraImpressora = '{$ateHoraImpressora}',
                    qualImpressora = 1 
